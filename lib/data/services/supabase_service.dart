...
class SupabaseService {
  ...
  Future<void> deleteAccount(String userId) async {
    try { await _client.from('water_logs').delete().eq('user_id', userId); } catch (_) {}
    try { await _client.from('sleep_logs').delete().eq('user_id', userId); } catch (_) {}
    try { await _client.from('chat_messages').delete().eq('user_id', userId); } catch (_) {}
    try { await _client.from('ai_chats').delete().eq('user_id', userId); } catch (_) {}
    try { await _client.from('users').delete().eq('id', userId); } catch (_) {}
    try { await _client.rpc('delete_user'); } catch (_) {}
    try { await _client.auth.signOut(); } catch (_) {}
  }
  ...
  Future<List<StepLog>> getStepLogsForRange(
      String userId, DateTime start, DateTime end) async {
    try {
      // Use date-only strings to match DATE column type in Supabase
