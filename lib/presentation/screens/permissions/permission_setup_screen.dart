...
class _PermissionSetupScreenState extends ConsumerState<PermissionSetupScreen> {
  ...
  Future<void> _requestHealthConnect() async {
    try {
      ...
      // Always re-check actual permissions — requestAuthorization can return false
      // even when user already granted permissions in Health Connect
      await Future.delayed(const Duration(milliseconds: 800));
      final recheckOk = await Health().hasPermissions(types,
          permissions: types.map((_) => HealthDataAccess.READ).toList()) ?? false;
      final granted = ok || recheckOk;
      if (mounted) setState(() => _hcGranted = granted);
      if (!granted) {
        ...
        final finalCheck = await Health().hasPermissions(types,
            permissions: types.map((_) => HealthDataAccess.READ).toList()) ?? false;
        if (mounted) setState(() => _hcGranted = finalCheck);
      }
    } catch (e) {
  ...
  Future<void> _requestPermission(_PermItem item) async {
    // Show rationale BEFORE system dialog — required by Google Play policy
    ...
    if (!shouldRequest) {
      setState(() => _granted[item.key] = false);
      return;
    }

    final status = await item.permission.request();
    if (mounted) setState(() => _granted[item.key] = status.isGranted);
    ...
    if (status.isPermanentlyDenied && mounted) {
      await showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: Text('${item.title} Permission Denied'),
          content: const Text(
              'This permission was denied. You can enable it anytime in '
              'Settings → Apps → HydroIQ → Permissions.'),
          actions: [
            TextButton(
  ...
  Future<void> _requestHealthConnectWithRationale() async {
    final shouldRequest = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
        title: const Row(children: [
          ...
          SizedBox(width: 10),
          Text('Health Connect', style: TextStyle(fontWeight: FontWeight.w700)),
        ]),
        content: const Text(
          'HydroIQ reads your steps and sleep data from Health Connect to give you '
          'accurate tracking and personalised hydration goals.\n\n'
          'We never upload your health data to third-party servers. '
          'All health data stays on your device or in your Supabase account.',
    ...
    if (shouldRequest) await _requestHealthConnect();
  }
          ...
          Expanded(child: ListView(
            ...
            children: [
              ...
              _HCPermCard(
                ...
                onAllow: _requestHealthConnectWithRationale,
