...
class _WaterScreenState extends ConsumerState<WaterScreen>
  ...
  // No hardcoded limit — daily cap is 1.5× the user's actual goal (safety buffer)
  ...
  Future<void> _addWithSound(int ml) async {
    ...
    final goalMl  = ref.read(smartGoalProvider);
    // Safety cap: 1.5× the goal (e.g. 6L goal → max 9L/day)
    final dailyCap = (goalMl * 1.5).round();
    if (total >= dailyCap) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('⚠️ Daily limit reached (${(dailyCap / 1000).toStringAsFixed(1)}L). Stay safe!'),
    ...
    final remaining = dailyCap - total;
  ...
  static String _weatherEmoji(String condition, double tempC) {
    ...
    return Scaffold(
      body: CustomScrollView(slivers: [
        SliverAppBar(
          ...
          actions: [
            // Weather button — always visible; shows temp when loaded, icon when loading
            GestureDetector(
              onTap: () {
                if (weather == null) {
                  // Re-trigger weather fetch if not yet loaded
                  _fetchWeather();
                }
              ...
              child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text(
                    weather != null
                        ? _weatherEmoji(weather.condition.toString(), (weather.temperatureC as num).toDouble())
                        : '🌤️',
                    style: const TextStyle(fontSize: 14)),
                  const SizedBox(width: 4),
                  Text(
                    weather != null
                        ? '${((weather.temperatureC as num).toDouble()).toStringAsFixed(0)}°C'
                        : 'Weather',
